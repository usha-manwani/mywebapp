    ...
    public class PopulateTree
    {
        ...
        private void PopulateTreeView(DataTable dtParent, int ParentId, TreeNode treeNode, TreeView t )
        {
            ...
            foreach (DataRow row in dtParent.Rows)
            {
                ...
                if (ParentId == 0)
                {
                    ...
                    DataTable dtChild = ExecuteCommand("Select Grade, ID, GradeID from Grade_Details where InsID ='" + val + "'");
                ...
                else
                {
                    if (ParentId !=0)
                    {
                        ...
                        DataTable dtclass = ExecuteCommand("Select Class, ID,ClassID from Class_Details where GradeID='" + val + "'");
        ...
        public Int32 InsertInstitute(string Text)
        {
            ...
            using (SqlConnection con = new SqlConnection(constr))
            {
                try
                {
                    using (SqlCommand cmd = new SqlCommand("sp_InstituteDetails", con))
                    {
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.Parameters.AddWithValue("@InstituteName", Text);
                        cmd.Parameters.Add("@InsId", SqlDbType.Int);
                        cmd.Parameters["@InsId"].Direction = ParameterDirection.Output;
                        con.Open();
                        cmd.ExecuteNonQuery();

                        t = Convert.ToInt32(cmd.Parameters["@InsId"].Value);
